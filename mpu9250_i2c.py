...
# Adapted from source https://makersportal.com/blog/2019/11/11/raspberry-pi-python-accelerometer-gyroscope-magnetometer
# Date: Jan 15 2020 PST
import smbus,time
import numpy as np
...
def gyro_caliberate():
    ...
    for i in range (1,200):
        ...
        gzbiasrec = np.append(gzbiasrec, gz)
        time.sleep(0.01)
    gx_bias = np.sum(gxbiasrec)/199.0
    gy_bias = np.sum(gybiasrec)/199.0
    gz_bias = np.sum(gzbiasrec)/199.0
...
# start I2C driver
print("[Boot] Initializing I2C Bus ...")
bus = smbus.SMBus(1) # start comm with i2c bus
print("[Boot] Initializing MPU9250 ...")
gyro_sens,accel_sens = MPU6050_start() # instantiate gyro/accel
time.sleep(1)
print("[Boot] Caliberating Gyro ...")
gxbias, gybias, gzbias = gyro_caliberate()
print("[Boot] Gyro Caliberated | Bias: {0:2.2f}, {1:2.2f}, {2:2.2f} ".format(gxbias, gybias, gzbias))
print("[Boot] MPU9250 Initialized")
